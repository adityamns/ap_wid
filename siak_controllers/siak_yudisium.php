...
class Siak_yudisium extends Siak_controller{
	...
	function index(){
	$this->siak_view->config = "Siak Widyatama - Generate Transkrip";
	
		$this->siak_view->judul = "Generate Transkrip";
			
		$this->siak_breadcrumbs->add(array('title'=>'Yudisium','href'=>'#'));
		$this->siak_breadcrumbs->add(array('title'=>'Generate Transkrip','href'=>'#'));
		...
		// $this->siak_datalist();
		$this->siak_view->siak_render('siak_yudisium/index', false);
	}
	...
	public function siak_datalist(){
		...
		WHERE a.status = '4'
		");
		$this->siak_view->siak_render('siak_yudisium/data', false);
	}
	function matkul($prodi,$semes){
		...
		$this->siak_view->siak_render('siak_yudisium/matkul', true);
	}
	public function siak_edit($nim){
		...
		$this->siak_view->siak_render('siak_yudisium/edit', true);
	}
	
	public function siak_edit_save($nim){
		$where = array('nim' => $nim);
		$this->siak_model->siak_edit_save($where);
		header('location: ' . URL . 'siak_yudisium');
	}
	function Data_IPK($prodi,$tahunid){
		...
		$this->siak_view->siak_render('siak_yudisium/list_data', true);
	}
	...
	public function simpan_ipk()
	{
		...
		$ga_jelas = $this->siak_model->siak_data_list("predikat","*");
			...
			foreach ($_POST[x] as $key => $value) {
				if($ipk[$key] >= $jelas['ipkmin'] && $ipk[$key] <= $jelas['ipkmax'] && $prodi_id[$key] == $jelas['prodi_id']){
					$grade = $jelas['nama'];
			$sql = "insert into transkrip_nilai (nim,cohort,prodi_id,ipk,predikat,ket,tahun_lulus) values ('".$nim[$key]."','".$cohort[$key]."','".$prodi_id[$key]."','".$ipk[$key]."','".$grade."','".$ket."','".$tahun_lulus."')";
				//echo $sql."<br>";
			$nimnim=$this->siak_model->siak_query("select","select * from transkrip_nilai where nim='".$nim[$key]."'");
			 if ($nimnim != NULL){
			     	echo "<script>alert('Sebelumnya, Data Sudah Pernah Di Generate');window.location.href='".URL."siak_yudisium';</script>";
			     	return 0;
			     }
			     else {
