...
class Siak_absensi_mahasiswa extends Siak_controller{
	...
	public function confirm_absen(){	
		...
		if($inout == 0){
			...
			$this->siak_model->siak_query("update","update absensi_dosen set waktu = '".$now."' where nip = '".$nip."' and tanggal = '".$waktu_kuliah[1]."';");
		}elseif($inout == 1){
			for ($i=0 ; $i < $total; $i++){
				$this->siak_model->siak_query("update","update absensi set status = ".$status[$i].", konfirmasi=1, waktu_keluar = '".$now."' where nim = '".$nim[$i]."' and tanggal = '".$waktu_kuliah[$i]."';");
				if($status[$i] != 1){
					$url = "siak_izin/index/".$nim[$i]."/".$_POST['cohort']."/".$_POST['tgl'];
					$this->siak_model->notifAbsen($nim[$i], $url);
				}
			}
			$this->siak_model->siak_query("update","update absensi_dosen set waktu_keluar = '".$now."' where nip = '".$nip."' and tanggal = '".$waktu_kuliah[1]."';");
		}	
	...
	public function DATA_KONFIRMASI(){
		...
		$prodi = "";
		$prodi_query = "select prodi_id from jadwal_kuliah where mulai = '".$_POST['jam_kuliah']."' and dosen_utama = '".$nip."'";
		$prodi_data = $this->siak_model->siak_query("select", $prodi_query);
		foreach($prodi_data as $key => $val) {
			$prodi = $val['prodi_id'];
		...
		if(isset($_POST['Filter'])){
			...
			/* input data mahasiswa sebelum absen */	
			$this->generate_absen();
			
			if(!empty($_POST['jam_kuliah'])){	
				
				/* untuk mendapatkan status absen (absen masuk/keluar) */
				$query_status_absen = "select distinct konfirmasi from absensi where tanggal = '".$_POST['jam_kuliah']."' and dosen_utama = '".$nip."' and kode_matkul = '".$matkul."'";
				$in_out = $this->siak_model->siak_query("select", $query_status_absen);
				foreach($in_out as $key => $val){
					$inout = $val['konfirmasi'];
				}
		...
		$sql_pilih = "SELECT * FROM jadwal_kuliah where mulai BETWEEN '".$tgl_awal."' AND '".$tgl_akhir."' and dosen_utama = '".$nip."'";
		//var_dump($sql_pilih);die();
		$this->siak_view->siak_pilih = $this->siak_model->siak_query("select", $sql_pilih);
		...
		$sql2 = "select a.kelas_id, b.nama_kelas from jadwal_kuliah a, master_kelas b where a.kelas_id = b.kelas_id and a.dosen_utama = '".$nip."' and a.mulai BETWEEN '".$tgl_awal."' AND '".$tgl_akhir."'";
		...
		$sql_ku = "SELECT a.*,to_char(waktu,'dd-mm-yyyy hh:mm:ss') AS time,nama_depan,nama_belakang,foto,konfirmasi FROM absensi a,data_pribadi_umum b WHERE a.nim=b.nim AND a.kode_matkul = '".$_POST['kode_matkul']."' AND a.tanggal = '".$_POST['jam_kuliah']."' AND a.kelas='".$_POST['kelas_id']."' AND a.prodi_id = '".$prodi_id."' ORDER BY a.nim ASC"; 			
	...
	public function generate_absen(){
		...
		if(empty($check_kelas)){
			$gen_absen_dosen_query = "insert into absensi_dosen (nip, prodi_id, cohort, kode_matkul, tanggal, status, pertemuanke, keterlambatan) values ('".$nip."','".$prodi."','0','".$_POST['kode_matkul']."', '".$_POST['jam_kuliah']."', '1', '0', '0')";
			//var_dump($gen_absen_dosen_query);die();
			$this->siak_model->siak_query("select", $gen_absen_dosen_query);

			$insert_kls = "SELECT DISTINCT result.kelas, result.nim, result.prodi_id, result.kode_matkul, result.mulai, result.pertemuanke, result.dosen_utama, result.cohort
							FROM (SELECT a.*, b.*
							...
							LEFT JOIN ( SELECT * FROM jadwal_kuliah WHERE mulai = '".$_POST['jam_kuliah']."' and dosen_utama = '".$nip."') b
			...
			foreach($this->siak_view->insert_kls as $key => $value) {
				...
				$sql_insert="insert into absensi (nim,prodi_id,cohort,kode_matkul,kode_topik,tanggal,status,pertemuanke,keterangan,catatan,konfirmasi,bukti_surat,edit_id,dosen_utama,kelas) values ('".$nim."','".$prodi."','".$cohort."','".$kd_matkul."','-','".$tanggal."',1,'".$pertemuan."',0,'-',0,'-',0,'".$nip."',".$kelas.")";						
