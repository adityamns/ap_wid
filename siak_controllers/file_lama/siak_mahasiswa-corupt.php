...
// 			else{
// 				$_POST['foto'] = $_FILES['userImage']['name'];
// 			}
		}
...
// 		die();
		...
		////skrip notipiskasi
		$url = 'siak_master/siak_mahasiswa/'.$_POST['nim'].'/'.$jenis.'#tab-pribadi';
		$pesan = Siak_session::siak_get('username').' telah melakukan perubahan data Pribadi';
		$dt = new DateTime();
		...
		$this->siak_model->insert_data($data_notif , "notification");
		
		$where = "datetime2 = '$datetime' ORDER BY id DESC";
		$id_notif = $this->siak_model->siak_getfield("id", "notification", $where);
		
		$users = $this->siak_model->siak_query("select", "select id from users where group_id <> 16");
	  
		  foreach($users as $row => $user){
			  $insert = "insert into notification_detail(id_notif, isread, table_name, field, value) values('".$id_notif."','f', 'users', 'id', '".$user['id']."')";
			  $this->siak_model->siak_query("insert" , $insert);
...
// 		var_dump($result);die();
		...
		if($result == NULL){
		    ...
		    echo '
		    <a class="btn green mini" data-toggle="modal" data-target="#approveDPU" onclick="approveDPU(\''.$id.'\',\''.$nim.'\',\''.$nama.'\',\''.$foto.'\')"><i class="icon-ok"></i> Approve</a>
		    <a class="btn red mini" data-toggle="modal" data-target="#hapusDPU" onclick="hapusDPU(\''.$id.'\',\''.$nim.'\',\''.$nama.'\',\''.$foto.'\')"><i class="icon-trash"></i> Disagree</a>
		    ';
		}
...
// 		die();
	...
	function tambah_bhs($nim,$jenis){
		...
		if($this->level == 16){
			...
			$url = 'siak_master/siak_mahasiswa/'.$_POST['nim'].'/'.$jenis.'#tab-bahasa-asing';
			$pesan = Siak_session::siak_get('username').' telah melakukan perubahan data Bahasa Asing';
			$dt = new DateTime();
			$datetime = $dt->format('Y-m-d H:i:s');
			$data_notif = array(
					    'datetime2' => $datetime,
...
// 		echo $sql_del;die();
	...
	public function siak_approve($nim, $table, $id, $jenis){
		...
		if($this->level != 16){
			...
			$users = $this->siak_model->siak_query("select", "select id from users where group_id = 16 and username = '$nim'");
