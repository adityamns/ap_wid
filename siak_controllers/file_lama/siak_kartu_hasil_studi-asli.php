...
// 			echo "<pre>";
// 			var_dump($this->siak_view->data_nilai);
// 			echo "</pre>";
		...
		foreach($this->siak_view->data_nilai as $ass => $hole){
			...
			$dosen_utama = $cek_ev_dos[0]['dosen_utama'];
			
			$asshole = $this->cek_ev_dos($dosen_utama,$hole['prodi_id'],$hole['matkul_id'],$hole['nim']);
...
// 			echo "</pre>";
			...
			// if($dosen_utama == $asshole[$ass]['nip'] && Siak_session::siak_get('level') == 16){
				// echo	"
					// <div class='alert alert-danger'>
						// Silahkan isi Evaluasi Dosen terlebih dahulu untuk mencetak KHS
						// <a href='".URL."siak_kartu_hasil_studi/ev_dos/".$nim."/".$value['prodi_id']."/".$semester."'>Klik Disini</a>
