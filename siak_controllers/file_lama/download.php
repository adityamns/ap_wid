	...
	function index($prodi_id = false, $smstr = false){
		...
		$sql = "
			...
			where 
				...
				dosen_matakuliah.kode_matkul = matakuliah.kode_matkul
				$semester
				$prodi
			";
			...
			$html .= '<table id = "download2" class="table table-striped table-bordered table-hover table-full-width">
				...
				<tbody>';
					...
					foreach ($this->siak_view->mhs as $key => $value) {
						...
						$html .= '<td><a class="btn green mini" data-toggle="modal" data-target="#modal-unduh" onclick="unduh(this)" link="'.URL.'download/unduh/'.$value['kode_matkul'].'/1"><i class="icon-download-alt"></i> Silabus</a></td>';
						$html .= '<td><a class="btn green mini" data-toggle="modal" data-target="#modal-unduh" onclick="unduh(this)" link="'.URL.'download/unduh/'.$value['kode_matkul'].'/2"><i class="icon-download-alt"></i> Materi</a></td>';
						$html .= "</tr>";
					$i++;}
				$html .= '</tbody>
			</table>';
