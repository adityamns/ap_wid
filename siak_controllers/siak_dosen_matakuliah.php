...
class Siak_dosen_matakuliah extends Siak_controller{
	...
	public function siak_datalist(){
		...
		if($prodi_id != NULL){
			...
			$new = implode(',', $new);
			$prodi = "where prodi_id in (".$new.")";
		    }else{
			$prodi = "where prodi_id in ('".$prodi_id."')";
		    }

			$sql = "
				  select * from dosen_matakuliah $prodi
			";
// 		    echo $sql;
// 		    die();
...
// 		$_POST['dosen_utama'] = implode(',', $_POST['dosen_utama']);
	...
	public function matkul($prodi,$smstr,$kurikulum_id){
		$this->siak_view->siak_prodi = $prodi;
		$this->siak_view->siak_data_list = $this->siak_model->siak_query("select", "SELECT * FROM matakuliah WHERE prodi_id = '".$prodi."' and kurikulum_id = '".$kurikulum_id."' and semester = '".$smstr."';");
