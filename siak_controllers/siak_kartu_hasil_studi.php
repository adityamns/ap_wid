...
class Siak_kartu_hasil_studi extends Siak_controller{
	...
	function index(){
		$this->siak_view->config = "Siak Widyatama - Kartu Hasil Studi";
...
// 		$nip = array();
		...
		$ad = implode(',', $mat);
		$dsa = $this->cek_ev_dos($prod,$ad,$ni);
		$nip = $this->data_matkul2($ad);
	...
	function cek_ev_dos($prodi_id,$matkul_id,$nim){
		$sql_cek_ev_dos = "select nip from evaluasi_dosen where prodi_id = '$prodi_id' and nim = '$nim' and kode_matkul in ($matkul_id) group by nip order by nip asc";
	...
	function data_matkul2($kode_matkul){
		...
		$sql = "
			...
			WHERE 
				...
				absensi_dosen.kode_matkul in ($kode_matkul) 
			GROUP BY 
