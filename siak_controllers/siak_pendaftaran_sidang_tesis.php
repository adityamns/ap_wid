...
class Siak_pendaftaran_sidang_tesis extends Siak_controller{
	...
	function index(){
		$this->siak_view->config = "Siak Widyatama - Pendaftaran Sidang Tesis";
	
		$this->siak_view->judul = "Pendaftaran Sidang Tesis";
			
		$this->siak_breadcrumbs->add(array('title'=>'Pembelajaran','href'=>'#'));
		$this->siak_breadcrumbs->add(array('title'=>'Tugas Akhir','href'=>'#'));
		$this->siak_breadcrumbs->add(array('title'=>'Pendaftaran Sidang Tesis','href'=>''. URL . 'siak_pendaftaran_sidang_tesis'));
		$this->siak_view->set_breadcrumbs($this->siak_breadcrumbs->output());
		if (Siak_session::siak_get('level')==16){
			$this->siak_view->siak_render('siak_pendaftaran_sidang_tesis/data_mhs', false);
		}
		else{
			foreach ($this->siak_session->siak_getAll() as $key => $value) {
				if ($value['groups'] == "tugas_akhir" && $value['kode'] == "pendaftaran_sidang_tesis") {
	...
	public function siak_datalist(){
		$this->siak_view->siak_render('siak_pendaftaran_sidang_tesis/data', false);
	}
	...
	public function siak_cek($nim, $semester){
		...
		$this->siak_view->data_judul = $this->siak_model->siak_query("select","select * from pendaftaran_tesis where nim='".$nim."' and status='2' and judultesis_id not in(".$isis.")");
		
		$this->siak_view->over = $this->siak_model->siak_query("select","select * from pendaftaran_tesis where nim='".$nim."' and status > '2' and judultesis_id not in(".$isis.")");
		...
		$this->siak_view->siak_render('siak_pendaftaran_sidang_tesis/sidang', true);
	}

	public function siak_ok($id){
		$this->siak_model->siak_query("update", "UPDATE pendaftaran_tesis set status = 3 WHERE id = ".$id.";");
		header('location: ' . URL . 'siak_pendaftaran_sidang_tesis');
	}
