...
class Siak_grafik_lagi extends Siak_controller{
	...
	function index(){
		...
		$mhs_pmb = "";
		foreach($this->siak_view->prodi as $pr => $odi){
			$pmb = $this->siak_model->siak_query("select","select count(program_studi_kode) from pmb_daftar where program_studi_kode='".$odi['prodi_id']."' and lulus=TRUE and undur_diri=FALSE and tahun='".$tahun."'");
			foreach($pmb as $pm => $b){
				$xyz = $b['count'];
			}
			$mhs_pmb = $mhs_pmb.$xyz.",";
		...
		$hpx = strlen($mhs_pmb);
		$hpxx = $hpx - 1;
		$hasilx = substr($mhs_pmb,0,$hpxx);
		$this->siak_view->pmb = $hasilx;
		...
		$this->siak_view->siak_render('siak_grafik_lagi/index', false);
