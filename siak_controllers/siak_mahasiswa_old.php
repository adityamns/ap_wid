...
// 		echo $sql;die();
		...
		if($_SESSION['level'] != 16 ){
			...
			/*$this->siak_view->config = "Siak Unhan - Data Mahasiswa";
	
			$this->siak_view->judul = "Data Mahasiswa";
			
			$this->siak_breadcrumbs->add(array('title'=>'Data Master','href'=>'#'));
			$this->siak_breadcrumbs->add(array('title'=>'Data Mahasiswa','href'=>''. URL . 'siak_mahasiswa'));
...
// 		die();
	...
	function add_pendidikan($jenis,$nim){
		$this->siak_view->jenis = $jenis;
		$this->siak_view->nim = $nim;
	...
	function cek_pendidikan( $nim, $jenis, $id, $bahasa, $edit_idx){
		...
		$sql = "select edit_id from pendidikan where nim = '$nim' and edit_id = '$id'";
