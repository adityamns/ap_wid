...
class Siak_jadwal extends Siak_controller{
	...
	public function siak_edit_save($id){
		...
		$absen = $this->siak_model->siak_query("select", "select * from absensi where kode_matkul = '".$data[0]->kode_matkul."' and pertemuanke = '".$data[0]->pertemuanke."' and status = 1");
		
		if(sizeof($absen) > 0){
			echo "Ada Mahasiswa yang sudah \nMengisi Absensi";
		}else{
// 			echo "Tidak";
// 			$this->siak_model->siak_query("update","update absensi set waktu = NULL, status = 4 where cohort = '".$data[0]->cohort."' and tanggal = '".$data[0]->mulai."' and kode_matkul = '".$data[0]->kode_matkul."' and pertemuanke = '".$data[0]->pertemuanke."'");
		}
...
// 		$this->siak_model->siak_edit_save($where);
		...
		$url = "siak_dashboard";
		$jadwal = $this->siak_model->siak_query("select", "select * from jadwal_kuliah where id = '$id'");
// 		$this->siak_model->notifJadwal($jadwal, $url, 'edit');
	...
	function load_jadwal($id,$jenis,$prodi,$kohort){
		...
		(SELECT jadwal_kuliah.id, matakuliah.nama_matkul AS title, jadwal_kuliah.mulai as START, jadwal_kuliah.akhir , NULL AS warna, jadwal_kuliah.tahun_id,jadwal_kuliah.ruang_id, jadwal_kuliah.dosen_utama,jadwal_kuliah.kurikulum_id,
