...
class Siak_atur_pembekalan extends Siak_controller{
	...
	function index(){
		$this->siak_view->config = "Siak Widyatama - Pengaturan Ruangan Pembekalan";
		
		$this->siak_view->judul = "Pengaturan Ruangan Pembekalan";
		
		$this->siak_breadcrumbs->add(array('title'=>'Pembelajaran','href'=>'#'));
		$this->siak_breadcrumbs->add(array('title'=>'Pembekalan','href'=>'#'));
		$this->siak_breadcrumbs->add(array('title'=>'Pengaturan Ruangan Pembekalan','href'=>'' .URL. 'siak_atur_pembekalan'));
...
// 		die();
		...
		// echo $nim;
// 		$sql_insert = "INSERT INTO atur_pembekalan( materi_id,prodi_id,ruang_id,jumlah_max,jumlah_mhs) VALUES('".$_POST['materi_id']."','".$_POST['prodi_id']."','".$_POST['ruang_id']."','".$_POST['jumlah_max']."','".$_POST['jumlah_mhs']."')";
		$this->siak_model->siak_create(); // ga jalan
...
// 				$this->siak_view->siak_data_list = $this->siak_model->siak_query("select", "SELECT a.nim, a.status, a.prodi_id, b.nama_depan, b.nama_belakang, b.foto, c.prodi FROM mahasiswa a, data_pribadi_umum b, prodi c WHERE a.nim = b.nim AND a.status != 2 AND a.prodi_id = c.prodi_id AND a.prodi_id = '".$key."' $nim UNION SELECT a.nim, a.status, a.prodi_id, b.nama_depan, b.nama_belakang, b.foto,  c.prodi FROM mahasiswa a, data_pribadi_pns b, prodi c WHERE a.nim = b.nim AND a.status != 2 AND a.prodi_id = c.prodi_id AND a.prodi_id = '".$key."' $nim ORDER BY nim ASC LIMIT ".$_POST['jumlah_mhs'].";");
				echo "SELECT a.nim, a.status, a.prodi_id, b.nama_depan, b.nama_belakang, b.foto, c.prodi FROM mahasiswa a, data_pribadi_umum b, prodi c WHERE a.nim = b.nim AND a.status != 2 AND a.prodi_id = c.prodi_id AND a.prodi_id = '".$key."' $nim UNION SELECT a.nim, a.status, a.prodi_id, b.nama_depan, b.nama_belakang, b.foto,  c.prodi FROM mahasiswa a, data_pribadi_pns b, prodi c WHERE a.nim = b.nim AND a.status != 2 AND a.prodi_id = c.prodi_id AND a.prodi_id = '".$key."' $nim ORDER BY nim ASC LIMIT ".$_POST['jumlah_mhs'].";";
			}
