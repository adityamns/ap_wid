		...
		if ($count > 0) {
			foreach ($statement as $key) {
				if (password_verify($_POST['password'], $key['password']) && $key['status'] == "1") {
					...
					$wuery_batas_waktu_absen = "select * from batas_waktu_absen";
					$batas = $this->siak_query("select",$wuery_batas_waktu_absen);
					foreach($batas as $row => $value){
						Siak_session::siak_set('batas', $value['waktu']);
					}
					...
					Siak_session::siak_set('ip_client',$_SERVER['REMOTE_ADDR']);
					/* var_dump($_SESSION['prodi']);
...
// 		echo "</pre>";
// 
// 		echo 'INSERT INTO '.$this->table.' ('.$field.') VALUES('.$valu.')';
// 		die();
