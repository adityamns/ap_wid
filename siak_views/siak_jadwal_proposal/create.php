...
<script>
	jQuery(document).ready(function() {
		...
		jQuery('#jadpro').fullCalendar({	
			...
			var view = jQuery('#jadpro').fullCalendar('getView');
			...
			if (allDay == false) {
				...
				var link="<?php echo URL; ?>siak_jadwal_proposal/form/<?php echo $this->prodi ?>";
				form_action(link);
			}
			...
			editable: true,
				...
				jQuery.ajax({
							url: '<?php echo URL; ?>siak_jadwal_proposal/siak_edit_save/'+event.id,
			...
			jQuery('#tanggal').datepicker({
				...
				jQuery('#jadpro').fullCalendar('gotoDate',new Date(Date.parse(dateText)));
				jQuery('#jadpro').fullCalendar('changeView','agendaWeek');
						...
						<div class="portlet-body light-grey">
							<div class="row-fluid">
								<div class="span3 responsive" data-tablet="span12 fix-margin" data-desktop="span8">
									...
									<h3 class="event-form-title">Jadwal Proposal</h3>
...
function simpan (link){
						var start = jQuery('#start').val();
							...
							jQuery.ajax({
								 //async: false,
								 url: '<?php echo URL; ?>siak_jadwal_proposal/cek_ruang',
