				...
				<tr>
					...
					<?php foreach ($this->sub_komponen as $key => $valus){
						if($vals['id']==$valus['id_komponen']){
							//var_dump($vals['id']);
					?>
					<input type="hidden" name="id_sub_edit[]" value="<?php echo $valus['id'];?>">
					<input type='text' class="m-wrap span8" name='sub_komponen_edit[]' value="<?php echo $valus['sub_komponen']; ?>"/>
					<button id='optx<?=$i?>' class='btn red mini' type='button' onClick="deleteThisVar(this);"><i class='icon-trash'></i> Batal</button>
					<?php 
						 }
						//else{
							...
							// </div>
							// <button class=" btn purple mini" type="button" id="fuck'.$vals['id'].'" onclick="damn2('.$vals['id'].')">Tambah Sub</button>
							// ';
							// break;
						// } 
					...
					<div id="variablegroup2<?=$i?>">
					</div><button class=" btn purple mini" type="button" id="fuck<?=$i?>" onclick="damn2(<?=$i?>)">Tambah Sub</button>
					</td>
