		...
		<tbody>
			...
			foreach ($this->siak_data as $key => $value) {
				...
				echo "<td>" . $value['prestasi'] . "
				      ...
				      <input type='hidden' class='nama' value='". $value['prestasi'] ."'>
...
function kirim_id(id,nim,nama){
	...
	$("#hapus").attr("href","<?php echo URL; ?>siak_mahasiswa/siak_delete/"+nim+"/prestasi/"+id+"/"+"<?php echo $this->jenis?>");
}
...
function kirim_id2(id,nim,nama){
	...
	$("#approve-data").attr("href","<?php echo URL; ?>siak_mahasiswa/siak_delete/"+nim+"/prestasi/"+id+"/"+"<?php echo $this->jenis?>");
}
