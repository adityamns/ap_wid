		...
		<table id="jabatan" class="table table-striped table-bordered table-hover table-full-width">
			...
			foreach ($this->siak_data as $key => $value) {
				...
				if($value['golongan'] != NULL){
				    echo ass($this->golongan, $value['golongan']);
				}else{
				    echo "<td></td>";
				}
...
// 				echo $value['golongan']!=""?"<td>" . $value['golongan'] . "</td>":"<td> </td>";
				...
				echo '
				      <a class="btn blue mini" data-toggle="modal" data-target="#editM" onclick="editModul(this)" link="'.URL.'siak_dosen/data_jabatan/'.$this->nip.'/edit/'.$value['id'].'"><i class="icon-edit"></i> Edit</a>
				      <a class="btn red mini" data-toggle="modal" data-target="#static" onclick="kirim_id(\''.$value['nip'].'\',\''.$value['id'].'\',\''.$value['nama_jabatan'].'\')"><i class="icon-trash"></i> Delete</a>
				      ';
