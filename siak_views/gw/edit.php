...
<!-- 					</td> -->
					...
					<td>
					  ...
					  <input type="checkbox" value="<?=$rec['reades']?>" <?=$reades?> onclick="click_checkbox(this,2)"/></td>
					<td>
					  <input type="hidden" name="allow_create_i[]" value="<?=$rec['creates']?>"/>
					  <input type="checkbox" value="<?=$rec['creates']?>" <?=$creates?> onclick="click_checkbox(this,3)"/>
					</td>
					<td>
					  <input type="hidden" name="allow_update_i[]" value="<?=$rec['updates']?>"/>
					  <input type="checkbox" value="<?=$rec['updates']?>" <?=$updates?> onclick="click_checkbox(this,4)"/>
					</td>
					<td>
					  <input type="hidden" name="allow_delete_i[]" value="<?=$rec['deletes']?>"/>
					  <input type="checkbox" value="<?=$rec['deletes']?>" <?=$deletes?> onclick="click_checkbox(this,5)"/>
					</td>
					<td>
					  <input type="hidden" name="pribadi_i[]" value="<?=$rec['loads']?>"/>
					  <input type="checkbox" value="<?=$rec['loads']?>" <?=$loads?> onclick="click_checkbox(this,6)"/>
					</td>
					<td>
...
<div class="modal-footer">
	...
	<button type="submit" class="btn green" onclick="document.getElementById('formEdit').submit();">Simpan</button>
</div>
</form>
