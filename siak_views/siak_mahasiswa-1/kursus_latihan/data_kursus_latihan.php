...
<script>
function kirim_idMKL(id,nim,nama){
	document.getElementById('data-MKL').innerHTML = "Anda akan menghapus <strong>"+nama+"</strong> dari tabel, klik Hapus untuk melanjutkan.";
	$("#hapus-MKL").attr("href","<?php echo URL; ?>siak_mahasiswa/siak_delete/"+nim+"/kursus_latihan/"+id+"/"+"<?php echo $this->jenis?>");
}

function kirim_id2MKL(id,nim,nama){
	document.getElementById('data-approve-MKL').innerHTML = "Anda akan menyetujui perubahan data <strong>"+nama+"</strong> dari tabel, klik Approve untuk melanjutkan.";
	$("#approve-data-MKL").attr("href","<?php echo URL; ?>siak_mahasiswa/siak_approve/"+nim+"/kursus_latihan/"+id+"/"+"<?php echo $this->jenis?>");
}
</script>
		...
		<table id="data_kursus" class="table table-striped table-bordered table-hover table-full-width">
			...
			<tbody>
				...
				foreach ($this->siak_data as $key => $value) {
					...
					echo "<td>" . $value['nama'] . "
					      <input type='hidden' class='idLat' value='". $value['id'] ."'>
					      <input type='hidden' class='edit_idLat' value='". $value['edit_id'] ."'>
					      <input type='hidden' class='hiddenLat' value='". $value['nomor_seleksi'] ."'>
					      <input type='hidden' class='nimLat' value='". $value['nim'] ."'>
					      <input type='hidden' class='namaLat' value='". $value['nama'] ."'>
					      <input type='hidden' class='jenisLat' value='". $this->jenis ."'>
					      </td>";
					echo "<td>" . $value['lama'] . "</td>";
					echo "<td>" . $value['tahun_selesai'] . "</td>";
					echo "<td>" . $value['tempat'] . "</td>";
					
					echo '<td class="statusLat">';
...
<div id="hapusMKL" class="modal hide fade" tabindex="-1" data-backdrop="static" data-keyboard="false">
	...
	<div class="modal-body">
		<span id="data-MKL"></span>
	</div>
	<div class="modal-footer">
		<button type="button" data-dismiss="modal" class="btn">Batal</button>
		<a type="button" class="btn green" id="hapus-MKL" href="#">Hapus</a>
	</div>
</div>
...
<div id="approveMKL" class="modal hide fade" tabindex="-1" data-backdrop="static" data-keyboard="false">
	...
	<div class="modal-body">
		<span id="data-approve-MKL"></span>
	</div>
	<div class="modal-footer">
		<button type="button" data-dismiss="modal" class="btn">Batal</button>
		<a type="button" class="btn green" id="approve-data-MKL" href="#">Approve</a>
	</div>
</div>
...
function loadDataK(){
    var id = $('.idLat');
    var edit_id = $('.edit_idLat');
    var nim = $('.nimLat');
    var jenis = $('.jenisLat');
    var bahasa = $('.namaLat');
    var statusLat = $('.statusLat');
	...
	$.ajax({
	    ...
	    row:i,
	    success:function(r){
		statusLat[this.row].innerHTML = r;
	    }
	});
    }
}
...
function EditKL(){
    ...
    $("#formEditKL").submit(); //Submit  the FORMu
