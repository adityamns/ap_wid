							  ...
							  <option value="">-- Pilih Prodi --</option>
								  <?php 
									  foreach($prodi_n as $keys => $val) {
										  ...
										  if($values['prodi_id'] == $val){
										$select = $value['prodi_id']==$values['prodi_id']?"selected":"";
										  echo "<option value='$values[prodi_id]' $select >$values[prodi]</option>";
										  }
									      }
									  }
								  ?>
 			...
 			<div class="row-fluid">
				<div class="span6 ">
					<div class="control-group">
						...
						<div class="controls">
      						  ...
      						  echo '<select class="m-wrap span12" name="kurikulum_id">
							<option>--Pilih--</option>
							';
							  foreach($this->kurikulum as $key => $val){
							    $prodi_new = explode(',', $val['prodi_id']);
							    $selected = ($val['kurikulum_id'] == $value['kurikulum_id'])?"selected":"";
							    if (in_array($prodi, $prodi_new)) {
								    echo "<option value='".$val['kurikulum_id']."' $selected>".$val['nama_kurikulum']."</option>";
							    }else{
								    echo "<option value='".$val['kurikulum_id']."' $selected>".$val['nama_kurikulum']."</option>";
							    }
							  }
				...
				<div class="span6">
					<div class="control-group">
						...
						<div class="controls">
							<select class="m-wrap span12" id='semester' link2="<?php echo URL;?>siak_dosen_matakuliah/siak_add" link="<?php echo URL;?>siak_dosen_matakuliah/matkul" onChange="getKurikulum(this)">
 			...
 			<div class="row-fluid">
				...
				<div class="span8">
					<div class="control-group">
						...
						<div id="<?php echo $i; ?>">
						<select name="dosen_pendamping[]" class="m-wrap span12">
...
function addVariable(){
	...
	var html = "<select name = 'dosen_utama[]' class='m-wrap span12'><?php foreach ($this->dosen_utama2 as $key => $val) { $prodi = explode(',', $val['prodi_ngajar']); /*if(in_array($_POST['prodi'],$prodi)){*/?><option value='<?php echo $val['nip']; ?>'><?php echo $val['nama']; ?></option><?php /*}*/ } ?> </select><button class='btn red mini' type='button' onClick=\"deleteThisVar(this);\">Hapus</button>";
