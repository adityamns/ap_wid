			...
			<div class="row-fluid">
				<div class="span6">
					<div class="control-group">
						...
						<div class="controls">
							<input type="text" class="m-wrap span12" id="awal" readonly value="<?=date("d-m-Y", strtotime($value['tanggal_mulai']))?>">
				...
				<div class="span6">
					<div class="control-group">
						...
						<div class="controls">
							<input type="text" class="m-wrap span12" id="akhir" readonly value="<?=date("d-m-Y", strtotime($value['tanggal_akhir']))?>">
			...
			<div class="row-fluid">
				<div class="span12">
					<div class="control-group">
						...
						<div class="controls">
							<textarea class="m-wrap span12" name="isi_acara" placeholder="Isi Acara..."><?=$value['isi_acara']?></textarea>
						</div>
					</div>
				</div>
			</div>
...
<!-- 					<th width="5%">Cohort</th> -->
				...
				<?php 
				$cohort = explode(',', $value['id_cohort']);
				foreach(explode(',', $value['prodi_id']) as $row => $rec){$i=0;
				?>
				<tr>
					<th width="10%">
					
					<select class="m-wrap span12" name="prodi_id[]"  link="<?php echo URL;?>siak_pengumuman/cohort">
					<?php foreach ($this->prodi as $key => $value) { 
					$selected = ($value['prodi_id'] == $rec )?"selected":"";
					?>
						<option value="<?php echo $value['prodi_id'];?>" <?=$selected?>><?php echo $value['prodi'];?></option>	
					<?php } ?>
					</select>
					
					</th>
<!-- 					<th width="5%"><?=$cohort[$row]?><br></th> -->
					<th width="5%">
						<a style="cursor:pointer;" class="btn red mini" title="Hapus" id="hapus_modul<?=$rec?>" onclick="hapusRow('<?=$rec?>')"><i class="icon-trash"></i> Hapus</a>
					</th>
				</tr>
				<?php $i++; } ?>
			</table>
 			...
 			<div id="umumz" style="display: none">
			<div class="row-fluid">
				<div class="span6">
					<div class="control-group">
						<label class="control-label" for="tanggal_mulai">Tanggal Mulai</label>
						<div class="controls">
							<input type="text" class="m-wrap span12" id="awal2" readonly>
			...
			<div class="row-fluid">
				<div class="span12">
					<div class="control-group">
						...
						<div class="controls">
							<textarea class="m-wrap span12" name="isi_acara" ></textarea>
						</div>
					</div>
				</div>
			</div>
...
function hapusRow(x){
	...
	{
		jQuery('#hapus_modul'+x).closest('tr').remove();
	}
}
