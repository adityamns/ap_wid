			...
			<table>
				<tbody>
					...
					foreach ($this->siak_data_list as $key => $value) {
						...
						$check=$value['status']!=3&&$value['status']!=2?"<select id='hadir' name='hadir[]' onchange='waktu(".$i.");'><option value='2'>Absensi</option><option value='1'>Hadir</option><option value='2'>Tidak Hadir</option></select>
						<input type='text' name='keterangan[]' placeholder='Keterangan..'>":"";
						echo "<input type='hidden' value='".$value['nim']."' name='nim[]'>";
						echo "<input type='hidden' id='waktu".$i."' name='waktu[]'>";
						$i++;
						if($i%2!=0)
						{
							echo '<tr><td>
